# -*- coding: utf-8 -*-
"""
面向不规则农田的无人机路径规划与药量优化模型 v9
- 独立可复现：本脚本内置农田空间建模逻辑，不依赖外部 4.py
- 病害扩散：高斯扩散模型 I_i(x)=R_i exp(-d^2/(2 sigma_i^2))，按阈值形成低/中/高斑块并裁剪到农田
- 风险融合：RF 气象风险 + XGBoost/GBDT 历史虫情风险 + 视觉风险，施药决策期动态权重
- 施药量：30 L 载药量约束，按面积、综合风险、置信度计算，沿内部覆盖路径均匀喷洒
- 路径：风险加权 K-means 分配 + 优先级/距离访问顺序 + 2-opt 局部优化
- 内部路径：喷幅 6 m，重叠系数 0.82，扫描线间距 4.92 m
- 资源：最大续航 25 min，安全阈值 3 min；安全药量阈值 3 L；匮乏点由模型触发
- 补给：道路候选点中优化选择 3 个补给点，最小化加权距离
运行：python recalculate_uav_model_v9.py
"""
...
OUT = BASE_DIR / 'uav_model_v9_results_beauty'
if OUT.exists():
    shutil.rmtree(OUT)
...
# 中文字体与论文图字号
for f in ['Microsoft YaHei', 'SimHei', 'Noto Sans CJK SC', 'Noto Sans CJK JP', 'Arial Unicode MS']:
    try:
...
df['害虫数值']=pd.to_numeric(df['害虫数值'], errors='coerce')
df['风险分位']=df.groupby(['害虫名称','采集方法'])['害虫数值'].rank(pct=True, method='average')
...
hdf=df.dropna(subset=['害虫名称','采集方法','观测年份','标准周','害虫数值']).copy()
gcols=['害虫名称','采集方法'] if '地点' not in hdf.columns else ['地点','害虫名称','采集方法']
hdf=hdf.sort_values(gcols+['观测年份','标准周']).reset_index(drop=True)
g=hdf.groupby(gcols, sort=False)['害虫数值']
...
week_cols=gcols+['标准周']; past_week=hdf.groupby(week_cols, sort=False)['害虫数值'].shift(1)
...
for i,(c,sc) in enumerate(zip(centers,scenario_labels),1):
    ...
    area_m2=total.area
    prior_risk=float(np.clip((W_WEATHER*w_risk + W_HISTORY*h_risk)/max(W_WEATHER+W_HISTORY,1e-9),0,1))
    prior_sigma=float(16 + 26*prior_risk)
    def prior_radius_for_tau(tau):
        return 0.0 if prior_risk<=tau else prior_sigma*math.sqrt(-2*math.log(tau/prior_risk))
    pr_low=prior_radius_for_tau(tau_low); pr_mid=prior_radius_for_tau(tau_mid); pr_high=prior_radius_for_tau(tau_high)
    prior_low_poly=c.buffer(max(pr_low,MIN_PATCH_RADIUS),resolution=48).intersection(field_area)
    prior_mid_poly=c.buffer(max(pr_mid,0.1),resolution=48).intersection(field_area) if pr_mid>0 else Polygon()
    prior_high_poly=c.buffer(max(pr_high,0.1),resolution=48).intersection(field_area) if pr_high>0 else Polygon()
    prior_mid_ring=prior_mid_poly.difference(prior_high_poly) if not prior_mid_poly.is_empty else Polygon()
    prior_low_ring=prior_low_poly.difference(prior_mid_poly) if not prior_mid_poly.is_empty else prior_low_poly
    prior_level='高风险' if prior_risk>=0.68 else ('中风险' if prior_risk>=0.45 else '低风险')
    ...
    regions.append(dict(id=f'D{i:02d}',center=c,x=c.x,y=c.y,scenario=sc,name=str(rng.choice(disease_names)),weather_risk=w_risk,history_risk=h_risk,visual_risk=visual,risk=risk,prior_risk=prior_risk,prior_level=prior_level,level=level,confidence=conf,lesion_ratio=lesion_ratio,density=density,sigma=sigma,prior_sigma=prior_sigma,total_geom=total,high_geom=high_poly,mid_geom=mid_ring,low_geom=low_ring,prior_high_geom=prior_high_poly,prior_mid_geom=prior_mid_ring,prior_low_geom=prior_low_ring,area_m2=area_m2,dose_l=dose_l,flow_rate_l_per_m=None,priority=None,spray_region=spray_region,**wvals,**hvals))
...
# -------------------- 路径最优性对照验证 --------------------
def order_nearest_only(regs):
    rem=regs[:]; cur=Point(takeoff.x,takeoff.y); order=[]
    while rem:
        j=min(range(len(rem)), key=lambda k: cur.distance(rem[k]['center']))
        order.append(rem.pop(j)); cur=order[-1]['center']
    return order

def order_risk_first(regs):
    return sorted(regs, key=lambda r: (-r['priority'], takeoff.distance(r['center'])))

def order_scan_x(regs):
    return sorted(regs, key=lambda r: (r['x'], r['y']))

def order_random(regs, seed):
    arr=regs[:]
    rng=np.random.default_rng(seed)
    rng.shuffle(arr)
    return arr

def build_uav_plan(strategy):
    plan=[]
    for i,(cid,_) in enumerate(cluster_info,1):
        regs=[r for r in regions if r['cluster']==cid]
        if strategy=='optimal':
            ordered=list([u for u in uavs if u['id']==f'UAV-{i}'][0]['regions'])
        elif strategy=='nearest':
            ordered=order_nearest_only(regs)
        elif strategy=='risk':
            ordered=order_risk_first(regs)
        elif strategy=='scan_x':
            ordered=order_scan_x(regs)
        elif strategy=='random':
            ordered=order_random(regs, 2026+i)
        else:
            ordered=list(regs)
        plan.append(dict(id=f'UAV-{i}', regions=ordered, color=['#1f77b4','#2ca02c','#9467bd'][i-1]))
    return plan

def run_strategy_validation(name, strategy):
    global uavs
    old_uavs=uavs
    uavs=build_uav_plan(strategy)
    init_defs,_,_,_,_,_=simulate(candidates, record_detail=False)
    alt_supply=optimize_supply_points(init_defs,3)
    alt_defs, alt_route_rows, _, _, _, _=simulate(alt_supply)
    alt_supply=optimize_supply_points(alt_defs,3)
    alt_defs, alt_route_rows, _, _, _, _=simulate(alt_supply)
    uavs=old_uavs
    df=pd.DataFrame(alt_route_rows)
    total_path=float(df['总执行路径_m'].sum())
    total_flight=float(df['区域间飞行距离_m'].sum())
    total_spray=float(df['内部喷洒路径_m'].sum())
    total_time=float(df['总仿真时间_min'].sum())
    finish_time=float(df['总仿真时间_min'].max())
    total_q=float(df['预计喷洒药量_L'].sum())
    battery_short=int(df['电量匮乏次数'].sum())
    pesticide_short=int(df['药量匮乏次数'].sum())
    supply_count=int(df['道路补给次数'].sum())
    return dict(
        路线方案=name,
        总执行路径_m=round(total_path,2),
        区域间飞行距离_m=round(total_flight,2),
        内部喷洒路径_m=round(total_spray,2),
        三机累计续航消耗_min=round(total_time,2),
        任务完成时间_min=round(finish_time,2),
        喷洒药量_L=round(total_q,2),
        电量匮乏次数=battery_short,
        药量匮乏次数=pesticide_short,
        资源匮乏总次数=battery_short+pesticide_short,
        道路补给次数=supply_count
    )

validation_rows=[
    run_strategy_validation('本文优化路线（加权聚类+风险最近邻+2-opt）','optimal'),
    run_strategy_validation('对照A：最近邻路线（不考虑风险权重）','nearest'),
    run_strategy_validation('对照B：高风险优先路线','risk'),
    run_strategy_validation('对照C：横向扫描路线','scan_x'),
    run_strategy_validation('对照D：随机访问路线','random')
]
best_path=validation_rows[0]['总执行路径_m']
best_time=validation_rows[0]['任务完成时间_min']
for row in validation_rows:
    row['相对本文路线增程_%']=round((row['总执行路径_m']/best_path-1)*100,2)
    row['相对本文路线完成时间增加_%']=round((row['任务完成时间_min']/best_time-1)*100,2)
validation_table=pd.DataFrame(validation_rows)

# -------------------- 输出表格 --------------------
region_table=pd.DataFrame([{
    '病害区编号':r['id'],'病害类别':r['name'],'中心X_m':round(r['x'],2),'中心Y_m':round(r['y'],2),'风险等级':r['level'],
    '综合风险值':round(r['risk'],4),'巡查前先验风险':round(r['prior_risk'],4),'气象风险':round(r['weather_risk'],4),'历史虫情风险':round(r['history_risk'],4),'视觉风险':round(r['visual_risk'],4),
...
metrics_table=pd.DataFrame([{'RF气象风险AUC':round(rf_auc,4),'历史虫情模型AUC':round(hist_auc,4),'视觉模型mAP@0.5':0.83729,'施药决策期气象权重':round(W_WEATHER,4),'施药决策期历史权重':round(W_HISTORY,4),'施药决策期视觉权重':round(W_VISUAL,4),'喷幅_m':WS,'重叠系数':RHO,'扫描线间距_m':round(SCAN_SPACING,2),'最大载药量_L':Q_MAX,'最大续航_min':T_MAX/60,'安全药量阈值_L':Q_SAFE,'安全电量阈值_min':T_SAFE/60,'区域间速度_m_s':VF,'喷洒速度_m_s':VW}])
for name,tab in [
    ...
    ('表9_不同路线策略仿真对比.csv',validation_table)
]:
...
def base_ax(title):
    ...
    ax.set_xlim(0,W); ax.set_ylim(0,H); ax.set_aspect('equal'); ax.set_xticks([]); ax.set_yticks([]); ax.set_title(title,fontsize=26)
    ...
    ax.text(takeoff.x+8,takeoff.y+8,'起飞点',fontsize=16,zorder=51)
    return fig,ax
...
def draw_disease(ax, labels=True):
    for r in regions:
        ...
        if labels: ax.text(r['x']+4,r['y']+4,r['id'],fontsize=14,zorder=21)

def draw_prior_risk_parcels(ax, labels=True):
    parcel_risks=[]
    for p in parcels:
        vals=[r['prior_risk'] for r in regions if p.contains(r['center']) or p.distance(r['center'])<1e-6]
        parcel_risks.append(max(vals) if vals else 0.0)
    cmap=mpl.colors.LinearSegmentedColormap.from_list('prior_reds',['#fff5f0','#fcbba1','#fb6a4a','#cb181d'])
    norm=mpl.colors.Normalize(vmin=0.0, vmax=max(parcel_risks) if max(parcel_risks)>0 else 1.0)
    for p,rv in zip(parcels,parcel_risks):
        if rv>0:
            alpha=0.10+0.36*(rv/max(parcel_risks)) if max(parcel_risks)>0 else 0.24
            add_geom(ax,p,cmap(norm(rv)),'#b02a2a',0.62,alpha,16)
    return cmap,norm

fig,ax=base_ax('巡查前气象-历史先验风险地块')
cmap,norm=draw_prior_risk_parcels(ax, True)
sm=mpl.cm.ScalarMappable(cmap=cmap,norm=norm)
sm.set_array([])
cbar=fig.colorbar(sm,ax=ax,fraction=0.032,pad=0.018)
cbar.set_label('先验风险值',fontsize=18)
cbar.ax.tick_params(labelsize=16)
fig.savefig(OUT/'图1b_巡查前气象历史先验病害扩散图.png',bbox_inches='tight'); plt.close(fig)

fig,ax=base_ax('高斯扩散病害斑块与起飞点')
draw_disease(ax, True)
ax.legend(handles=[Patch(facecolor='#c92a2a',label='高强度斑块'),Patch(facecolor='#f6a04d',label='中强度斑块'),Patch(facecolor='#ffe084',label='低强度斑块'),Line2D([0],[0],marker='*',color='w',markerfacecolor='#d4a017',markeredgecolor='black',markersize=12,label='起飞点')],loc='center left',bbox_to_anchor=(1.02,0.5),fontsize=18)
...
for i,p in enumerate(supply_points,1):
    ax.scatter([p.x],[p.y],marker='s',s=72,c='#006d77',edgecolors='white',linewidths=0.8,zorder=50); ax.text(p.x+6,p.y+6,f'S{i}',fontsize=14,color='#003b3f',zorder=51)
for i,d in enumerate(defs,1):
    ...
    ax.scatter([d['point'].x],[d['point'].y],marker=marker,s=75,c=col,edgecolors='white',linewidths=0.7,zorder=55); ax.text(d['point'].x+5,d['point'].y+5,f'Q{i}',fontsize=14,color=col,zorder=56)
ax.legend(handles=[Line2D([0],[0],marker='s',color='w',markerfacecolor='#006d77',markersize=8,label='道路补给点'),Line2D([0],[0],marker='^',color='w',markerfacecolor='#5a189a',markersize=8,label='电量匮乏'),Line2D([0],[0],marker='X',color='w',markerfacecolor='#d00000',markersize=8,label='药量匮乏'),Line2D([0],[0],marker='D',color='w',markerfacecolor='#ee9b00',markersize=8,label='双重匮乏')],loc='center left',bbox_to_anchor=(1.02,0.5),fontsize=18)
...
for r in regions:
    ax.text(r['x']+3,r['y']-6,f"{r['uav'][-1]}-{r['visit_order']}",fontsize=14,color='black',zorder=40)
for i,p in enumerate(supply_points,1):
    ax.scatter([p.x],[p.y],marker='s',s=72,c='#006d77',edgecolors='white',linewidths=0.8,zorder=50); ax.text(p.x+6,p.y+6,f'S{i}',fontsize=14,color='#003b3f',zorder=51)
...
def plot_resource_curve(col, ylabel, title, filename):
    ...
    ax.grid(alpha=0.25); ax.legend(loc='best',fontsize=16)
...
ax.set_xlim(max(0,minx-pad),min(W,maxx+pad)); ax.set_ylim(max(0,miny-pad),min(H,maxy+pad)); ax.set_aspect('equal'); ax.set_xticks([]); ax.set_yticks([]); ax.set_title(f"单病害区内部覆盖喷洒路径：{single['id']}",fontsize=24)
for s in ax.spines.values(): s.set_visible(False)
...
if single['entry'] is not None:
    ax.scatter([single['entry'].x],[single['entry'].y],marker='*',s=180,c='#d4a017',edgecolors='black',linewidths=0.6,zorder=30); ax.text(single['entry'].x+3,single['entry'].y+3,'入口',fontsize=16)
if single['exit'] is not None:
    ax.scatter([single['exit'].x],[single['exit'].y],marker='o',s=70,c='#355c7d',edgecolors='white',linewidths=0.6,zorder=30); ax.text(single['exit'].x+3,single['exit'].y+3,'出口',fontsize=16)
ax.legend(handles=[Patch(facecolor='#c7e48b',alpha=0.55,label='安全缓冲喷洒区'),Patch(facecolor='#ffe084',label='病害斑块'),Line2D([0],[0],color='#b22222',lw=2,label='往复式喷洒航线')],loc='center left',bbox_to_anchor=(1.02,0.5),fontsize=18)
fig.savefig(OUT/'图4_单病害区内部喷洒路径图.png',bbox_inches='tight'); plt.close(fig)

# -------------------- 论文增强可视化 --------------------
def shortage_style(dtype):
    if '电量' in dtype and '药量' in dtype:
        return 'D', '#ee9b00', '双重匮乏'
    if '电量' in dtype:
        return '^', '#5a189a', '电量匮乏'
    return 'X', '#d00000', '药量匮乏'

fig,ax=base_ax('道路补给点服务分区与匮乏点权重')
draw_disease(ax, False)
xs=np.linspace(0,W,260); ys=np.linspace(0,H,260)
xx,yy=np.meshgrid(xs,ys)
sp_coords=np.array([[p.x,p.y] for p in supply_points])
nearest=np.stack([(xx-x)**2+(yy-y)**2 for x,y in sp_coords],axis=0).argmin(axis=0)
ax.contourf(xx,yy,nearest,levels=[-0.5,0.5,1.5,2.5],colors=['#d7f0f7','#e3f3d4','#f8e5c1'],alpha=0.23,zorder=2)
if len(def_table)>0:
    w=def_table['权重'].to_numpy(float)
    sizes=80+(w-w.min())/(w.max()-w.min()+1e-9)*260
    for i,d in enumerate(defs):
        near_idx=int(np.argmin([d['point'].distance(p) for p in supply_points]))
        sp=supply_points[near_idx]
        ax.plot([d['point'].x,sp.x],[d['point'].y,sp.y],color='#52616b',lw=0.85,alpha=0.55,zorder=42)
    for dtype,sub in def_table.groupby('匮乏类型'):
        marker,col,label=shortage_style(dtype)
        idx=sub.index.to_numpy()
        ax.scatter(sub['匮乏点X_m'],sub['匮乏点Y_m'],s=sizes[idx],marker=marker,c=col,edgecolors='white',linewidths=0.8,zorder=55,label=label)
    for _,row in def_table.iterrows():
        ax.text(row['匮乏点X_m']+5,row['匮乏点Y_m']+5,row['匮乏点编号'],fontsize=14,color='#333333',zorder=56)
for i,p in enumerate(supply_points,1):
    ax.scatter([p.x],[p.y],marker='s',s=90,c='#006d77',edgecolors='white',linewidths=0.8,zorder=60)
    ax.text(p.x+7,p.y+7,f'S{i}',fontsize=16,color='#003b3f',weight='bold',zorder=61)
handles=[Line2D([0],[0],marker='s',color='w',markerfacecolor='#006d77',markeredgecolor='white',markersize=8,label='道路补给点'),
         Line2D([0],[0],marker='^',color='w',markerfacecolor='#5a189a',markeredgecolor='white',markersize=8,label='电量匮乏'),
         Line2D([0],[0],marker='X',color='w',markerfacecolor='#d00000',markeredgecolor='white',markersize=8,label='药量匮乏'),
         Line2D([0],[0],marker='D',color='w',markerfacecolor='#ee9b00',markeredgecolor='white',markersize=8,label='双重匮乏')]
ax.legend(handles=handles,loc='center left',bbox_to_anchor=(1.02,0.5),fontsize=16,frameon=True)
fig.savefig(OUT/'图7_补给点Voronoi归属图_美化版.png',bbox_inches='tight'); plt.close(fig)

fig,ax=plt.subplots(figsize=(10.5,4.8),dpi=180)
event_colors={'电量不足':'#5a189a','药量不足':'#d00000','电量不足+药量不足':'#ee9b00','道路补给':'#006d77'}
event_markers={'电量不足':'^','药量不足':'X','电量不足+药量不足':'D','道路补给':'s'}
ypos={'UAV-1':3,'UAV-2':2,'UAV-3':1}
max_t=max(1,float(event_table['累计时间_min'].max()) if len(event_table)>0 else 1)
for u,y in ypos.items():
    ax.hlines(y,0,max_t+6,color='#ccd3d8',lw=1.4,zorder=1)
    sub=route_table[route_table['无人机编号']==u]
    if len(sub)>0:
        ax.text(max_t+6.5,y,f"{float(sub['总仿真时间_min'].iloc[0]):.1f} min",va='center',fontsize=16,color='#555')
for _,row in event_table.iterrows():
    etype=row['匮乏类型'] if row['事件类型']=='资源匮乏' else '道路补给'
    ax.scatter(row['累计时间_min'],ypos[row['无人机编号']],marker=event_markers.get(etype,'o'),s=90,c=event_colors.get(etype,'#333'),edgecolors='white',linewidths=0.8,zorder=5)
    ax.text(row['累计时间_min'],ypos[row['无人机编号']]+0.13,row['事件编号'],fontsize=14,ha='center',color='#444')
ax.set_yticks([1,2,3]); ax.set_yticklabels(['UAV-3','UAV-2','UAV-1'])
ax.set_xlim(-1,max_t+14); ax.set_ylim(0.5,3.75)
ax.set_xlabel('累计时间 / min'); ax.set_title('资源匮乏与补给事件时间轴')
ax.grid(axis='x',alpha=0.25)
handles=[Line2D([0],[0],marker=event_markers[k],color='w',markerfacecolor=v,markeredgecolor='white',markersize=8,label=k.replace('不足','匮乏')) for k,v in event_colors.items()]
ax.legend(handles=handles,loc='upper center',bbox_to_anchor=(0.5,-0.13),ncol=4,fontsize=16,frameon=False)
fig.savefig(OUT/'图8_资源触发时间轴_美化版.png',bbox_inches='tight'); plt.close(fig)

fig,axes=plt.subplots(2,2,figsize=(10.2,7.2),dpi=180)
metrics=[('区域间飞行距离_m','区域间飞行距离 / m'),('内部喷洒路径_m','内部喷洒路径 / m'),('预计喷洒药量_L','预计喷洒药量 / L'),('道路补给次数','道路补给次数')]
for ax,(col,title) in zip(axes.ravel(),metrics):
    vals=route_table[col].to_numpy(float)
    names=route_table['无人机编号'].tolist()
    bars=ax.bar(names,vals,color=[u['color'] for u in uavs],alpha=0.86,width=0.58)
    ax.set_title(title,fontsize=20); ax.grid(axis='y',alpha=0.22)
    ax.spines[['top','right']].set_visible(False)
    for b,v in zip(bars,vals):
        ax.text(b.get_x()+b.get_width()/2,b.get_height()*1.015,f'{v:.1f}',ha='center',va='bottom',fontsize=16)
fig.suptitle('三台无人机任务负载对比',fontsize=24)
fig.tight_layout()
fig.savefig(OUT/'图9_UAV任务负载对比图_美化版.png',bbox_inches='tight'); plt.close(fig)

fig,ax=plt.subplots(figsize=(13.0,8.6),dpi=220)
risk_color={'低风险':'#6abf69','中风险':'#f4a261','高风险':'#d64b35'}
for level in ['低风险','中风险','高风险']:
    sub=region_table[region_table['风险等级']==level]
    if len(sub)==0:
        continue
    sizes=110+sub['预计药量_L']/max(region_table['预计药量_L'].max(),1e-9)*520
    ax.scatter(sub['综合风险值'],sub['病害面积_ha'],s=sizes,c=risk_color.get(level,'#999'),
               alpha=0.70,edgecolors='white',linewidths=1.2,label=level,zorder=3)

xmin,xmax=region_table['综合风险值'].min(),region_table['综合风险值'].max()
ymin,ymax=region_table['病害面积_ha'].min(),region_table['病害面积_ha'].max()
xpad=max(0.035,(xmax-xmin)*0.12)
ypad=max(0.08,(ymax-ymin)*0.12)
ax.set_xlim(max(0,xmin-xpad),min(1,xmax+xpad))
ax.set_ylim(ymin-ypad*1.2,ymax+ypad)
xrange=ax.get_xlim()[1]-ax.get_xlim()[0]
yrange=ax.get_ylim()[1]-ax.get_ylim()[0]

label_rows=region_table.sort_values(['病害面积_ha','综合风险值'],ascending=[False,False]).reset_index(drop=True)
points=np.array([[float(r['综合风险值']),float(r['病害面积_ha'])] for _,r in label_rows.iterrows()])
center=np.array([float(region_table['综合风险值'].median()),float(region_table['病害面积_ha'].median())])
labels=[]
for i,p0 in enumerate(points):
    ...
    tangent=np.array([-direction[1],direction[0]])*side
    offset=direction*np.array([0.018*xrange,0.035*yrange])+tangent*np.array([0.012*xrange,0.020*yrange])
    labels.append(p0+offset)
labels=np.array(labels)
low=np.array([ax.get_xlim()[0]+0.012*xrange,ax.get_ylim()[0]+0.018*yrange])
high=np.array([ax.get_xlim()[1]-0.012*xrange,ax.get_ylim()[1]-0.018*yrange])
labels=np.minimum(np.maximum(labels,low),high)
min_dx=0.038*xrange
min_dy=0.040*yrange
for _ in range(220):
    for i in range(len(labels)):
        for j in range(i+1,len(labels)):
            ...
            if abs(dx)<min_dx and abs(dy)<min_dy:
                push=np.array([np.sign(dx) if dx!=0 else (1 if i%2==0 else -1),
                               np.sign(dy) if dy!=0 else (1 if i%3==0 else -1)],dtype=float)
                move=np.array([(min_dx-abs(dx))*0.10,(min_dy-abs(dy))*0.10])*push
                labels[i]+=move
                labels[j]-=move
    labels += (points-labels)*0.002
    labels=np.minimum(np.maximum(labels,low),high)
for (_,row),p0,lp in zip(label_rows.iterrows(),points,labels):
    ax.annotate(row['病害区编号'],xy=(p0[0],p0[1]),xytext=(lp[0],lp[1]),fontsize=14,color='#2f2f2f',
                ha='center',va='center',zorder=5,
                arrowprops=dict(arrowstyle='-',color='#8a8a8a',lw=0.8,alpha=0.60))

ax.set_xlabel('综合风险值'); ax.set_ylabel('病害面积 / ha'); ax.set_title('风险-面积-药量气泡图')
ax.grid(alpha=0.25); ax.spines[['top','right']].set_visible(False)
ax.legend(title='风险等级',loc='center left',bbox_to_anchor=(1.02,0.5),frameon=True,fontsize=16)
fig.savefig(OUT/'图10_风险药量气泡图_美化版.png',bbox_inches='tight'); plt.close(fig)

# 不同路线策略验证图：按期刊多面板风格组织，突出有差异的验证指标。
fig = plt.figure(figsize=(19.5, 11.2), dpi=220)
gs = fig.add_gridspec(2, 6, hspace=0.86, wspace=0.92)
short_names = ['本文优化', '最近邻', '风险优先', '横向扫描', '随机']
main_color = '#0b7a7f'
validation_palette = ['#0b7a7f', '#a9d3df', '#e7b89d', '#b9d9b6', '#c9c7da']
line_colors = ['#0b7a7f', '#d88c5a', '#6f8f72', '#7a74a8']

def panel_label(ax, label):
    ax.text(-0.13, 1.08, label, transform=ax.transAxes, fontsize=26, weight='bold', va='top', ha='left')

def journal_axis(ax, grid_axis='x'):
    ax.spines[['top', 'right']].set_visible(False)
    ax.spines['left'].set_linewidth(0.8)
    ax.spines['bottom'].set_linewidth(0.8)
    ax.grid(axis=grid_axis, color='#aeb7b7', alpha=0.24, linewidth=0.6)
    ax.tick_params(axis='both', labelsize=16, width=0.8)

def hbar_panel(ax, col, title, xlabel, label):
    vals = validation_table[col].to_numpy(float)
    y = np.arange(len(short_names))
    ax.barh(y, vals, color=validation_palette, edgecolor='white', height=0.58, linewidth=0.8)
    ax.set_yticks(y); ax.set_yticklabels(short_names, fontsize=16)
    ax.invert_yaxis()
    span = vals.max() - vals.min()
    ax.set_xlim(vals.min() - span*0.18, vals.max() + span*0.18)
    for yi, v in zip(y, vals):
        text = f'{v:.1f}' if 'min' in xlabel else f'{v:.0f}'
        ax.text(v + span*0.025, yi, text, va='center', fontsize=16, color='#222')
    ax.axvline(vals[0], color=main_color, lw=1.0, ls='--', alpha=0.58)
...
hbar_panel(ax, '总执行路径_m', '不同策略总执行路径', '总执行路径 / m', 'a')

ax = fig.add_subplot(gs[0, 2:4])
norm_cols = ['总执行路径_m', '区域间飞行距离_m', '三机累计续航消耗_min', '任务完成时间_min']
norm_labels = ['总路径', '区域间飞行', '续航消耗', '完成时间']
x = np.arange(len(short_names))
for col, lab, colr in zip(norm_cols, norm_labels, line_colors):
    vals = validation_table[col].to_numpy(float)
    norm = vals / vals[0]
    ax.plot(x, norm, marker='o', markersize=3.8, lw=1.15, color=colr, label=lab)
ax.axhline(1.0, color='#777777', lw=0.8, ls='--', alpha=0.55)
ax.set_xticks(x); ax.set_xticklabels(short_names, rotation=22, ha='right')
ax.set_ylabel('相对本文路线比值', fontsize=18)
ax.set_title('多指标标准化对比', fontsize=18, pad=7)
journal_axis(ax, 'y')
ax.legend(fontsize=14, frameon=False, loc='upper left')
panel_label(ax, 'b')

ax = fig.add_subplot(gs[0, 4:6])
hbar_panel(ax, '区域间飞行距离_m', '区域间飞行距离', '区域间飞行距离 / m', 'c')

ax = fig.add_subplot(gs[1, 0:3])
delta = validation_table['相对本文路线增程_%'].to_numpy(float)
y = np.arange(len(short_names))
ax.barh(y, delta, color=validation_palette, edgecolor='white', height=0.58, linewidth=0.8)
ax.set_yticks(y); ax.set_yticklabels(short_names, fontsize=16)
ax.invert_yaxis()
ax.set_xlim(0, max(delta)*1.18 + 0.05)
for yi, v in zip(y, delta):
    ax.text(v + 0.05, yi, f'{v:.2f}%', va='center', fontsize=16, color='#222')
ax.set_title('相对增程幅度', fontsize=18, pad=7)
ax.set_xlabel('相对本文路线增程 / %', fontsize=18)
journal_axis(ax, 'x')
panel_label(ax, 'd')

ax = fig.add_subplot(gs[1, 3:6])
battery = validation_table['电量匮乏次数'].to_numpy(float)
pesticide = validation_table['药量匮乏次数'].to_numpy(float)
yy = np.arange(len(short_names))
ax.barh(yy, battery, color='#7a55b7', height=0.58, edgecolor='white', linewidth=0.8, label='电量匮乏')
ax.barh(yy, pesticide, left=battery, color='#d95f5f', height=0.58, edgecolor='white', linewidth=0.8, label='药量匮乏')
for yi, (b, q) in enumerate(zip(battery, pesticide)):
    if b > 0:
        ax.text(b/2, yi, f'{int(b)}', ha='center', va='center', fontsize=14, color='white')
    if q > 0:
        ax.text(b+q/2, yi, f'{int(q)}', ha='center', va='center', fontsize=14, color='white')
    ax.text(b+q+0.15, yi, f'{int(b+q)}', ha='left', va='center', fontsize=16, color='#222')
ax.set_yticks(yy); ax.set_yticklabels(short_names, fontsize=16)
ax.invert_yaxis()
ax.set_xlim(0, max(battery+pesticide)+1.0)
ax.set_title('资源匮乏触发组成', fontsize=18, pad=24)
ax.set_xlabel('触发次数', fontsize=18)
journal_axis(ax, 'x')
ax.legend(fontsize=14, frameon=False, loc='lower center', bbox_to_anchor=(0.5, 1.01), ncol=2, borderaxespad=0.0)
panel_label(ax, 'e')

fig.suptitle('不同路线策略的飞行过程仿真验证', fontsize=26, weight='bold', y=0.965)
fig.subplots_adjust(top=0.86, bottom=0.10, left=0.08, right=0.98)
fig.savefig(OUT/'图11_不同路线策略仿真验证对比图.png', bbox_inches='tight')
fig.savefig(OUT/'图11_不同路线策略仿真验证对比图_顶刊风格.png', bbox_inches='tight')
plt.close(fig)

# -------------------- 图7b：保留简洁版，另绘农田背景详细版 --------------------
fig, ax = base_ax('补给点服务分区与资源匮乏点（农田背景详细版）')
draw_disease(ax, False)

xs = np.linspace(0, W, 260)
ys = np.linspace(0, H, 260)
xx, yy = np.meshgrid(xs, ys)
sp_coords = np.array([[p.x, p.y] for p in supply_points])
nearest = np.stack([(xx - x) ** 2 + (yy - y) ** 2 for x, y in sp_coords], axis=0).argmin(axis=0)
ax.contourf(
    ...
    colors=['#d9f0f5', '#e7f4d8', '#fde7c4'],
    alpha=0.10,
    zorder=2
)

if len(defs) > 0:
    weights = np.array([float(d.get('weight', 1.0)) for d in defs])
    sizes = 110 + (weights - weights.min()) / (weights.max() - weights.min() + 1e-9) * 360

    for d in defs:
        near_idx = int(np.argmin([d['point'].distance(p) for p in supply_points]))
        sp = supply_points[near_idx]
        ax.plot(
            [d['point'].x, sp.x], [d['point'].y, sp.y],
            color='#4b5963', lw=1.0, alpha=0.42, zorder=44
        )

    for i, d in enumerate(defs, 1):
        dtype = d['type']
        if '电量' in dtype and '药量' not in dtype:
            marker, color, label = '^', '#6f2dbd', '电量匮乏'
        elif '药量' in dtype and '电量' not in dtype:
            marker, color, label = 'X', '#d00000', '药量匮乏'
        else:
            marker, color, label = 'D', '#ee9b00', '双重匮乏'
        ax.scatter(
            [d['point'].x], [d['point'].y],
            s=float(sizes[i - 1]), marker=marker, c=color,
            edgecolors='white', linewidths=1.2, zorder=62
        )
        ax.text(
            d['point'].x + 7, d['point'].y + 7, f'Q{i:02d}',
            fontsize=14, color='#303030', zorder=63
        )

for i, p in enumerate(supply_points, 1):
    ax.scatter(
        [p.x], [p.y],
        marker='s', s=120, c='#00727d',
        edgecolors='white', linewidths=1.1, zorder=66
    )
    ax.text(p.x + 8, p.y + 8, f'S{i}', fontsize=18, color='#003b3f', weight='bold', zorder=67)

handles = [
    Patch(facecolor='#d9f0f5', edgecolor='none', alpha=0.5, label='S1服务区'),
    Patch(facecolor='#e7f4d8', edgecolor='none', alpha=0.5, label='S2服务区'),
    Patch(facecolor='#fde7c4', edgecolor='none', alpha=0.5, label='S3服务区'),
    Line2D([0], [0], marker='s', color='w', markerfacecolor='#00727d', markeredgecolor='white', markersize=9, label='道路补给点'),
    Line2D([0], [0], marker='^', color='w', markerfacecolor='#6f2dbd', markeredgecolor='white', markersize=9, label='电量匮乏'),
    Line2D([0], [0], marker='X', color='w', markerfacecolor='#d00000', markeredgecolor='white', markersize=9, label='药量匮乏'),
    Line2D([0], [0], marker='D', color='w', markerfacecolor='#ee9b00', markeredgecolor='white', markersize=9, label='双重匮乏'),
    Line2D([0], [0], color='#4b5963', lw=1.0, alpha=0.55, label='最近补给归属')
]
ax.legend(handles=handles, loc='center left', bbox_to_anchor=(1.02, 0.5), fontsize=16, frameon=True)
ax.set_xlim(-45, W + 45)
ax.set_ylim(-90, H + 35)
fig.savefig(OUT / '图7b_补给点Voronoi归属图_农田背景详细版.png', bbox_inches='tight')
plt.close(fig)

# 保留用户认可的简洁图7：不叠加农田背景，突出补给归属与三类匮乏点。
fig, ax = plt.subplots(figsize=(8.8, 8.1), dpi=180)
...
ax.grid(True, color='#d8ded6', alpha=0.36, linewidth=0.75)

if len(defs) > 0:
    weights = np.array([float(d.get('weight', 1.0)) for d in defs])
    sizes = 125 + (weights - weights.min()) / (weights.max() - weights.min() + 1e-9) * 430
    ...
    for d in defs:
        ...
        ax.plot(
            [d['point'].x, sp.x], [d['point'].y, sp.y],
            color='#73808a', lw=1.1, alpha=0.5, zorder=3
        )
    ...
    for i, d in enumerate(defs, 1):
        ...
        if '电量' in dtype and '药量' not in dtype:
            ...
            marker, color = 'X', '#d00000'
        else:
            marker, color = 'D', '#ee9b00'
        ax.scatter(
            [d['point'].x], [d['point'].y],
            s=float(sizes[i - 1]), marker=marker, c=color,
            edgecolors='white', linewidths=1.3, zorder=7
        )
        ax.text(d['point'].x + 7, d['point'].y + 7, f'Q{i:02d}', fontsize=16, color='#3a3a3a', zorder=8)

for i, p in enumerate(supply_points, 1):
    ax.scatter([p.x], [p.y], marker='s', s=140, c='#00727d', edgecolors='white', linewidths=1.3, zorder=9)
    ax.text(p.x + 9, p.y + 9, f'S{i}', fontsize=18, color='#003b3f', weight='bold', zorder=10)
...
ax.set_xlabel('X / m')
ax.set_ylabel('Y / m')
ax.set_title('补给点Voronoi归属与模型触发匮乏点')
handles = [
    Patch(facecolor='#d9f0f5', edgecolor='none', alpha=0.7, label='S1服务区'),
    Patch(facecolor='#e7f4d8', edgecolor='none', alpha=0.7, label='S2服务区'),
    Patch(facecolor='#fde7c4', edgecolor='none', alpha=0.7, label='S3服务区'),
    Line2D([0], [0], marker='s', color='w', markerfacecolor='#00727d', markeredgecolor='white', markersize=9, label='道路补给点'),
    Line2D([0], [0], marker='^', color='w', markerfacecolor='#6f2dbd', markeredgecolor='white', markersize=9, label='电量匮乏'),
    Line2D([0], [0], marker='X', color='w', markerfacecolor='#d00000', markeredgecolor='white', markersize=9, label='药量匮乏'),
    Line2D([0], [0], marker='D', color='w', markerfacecolor='#ee9b00', markeredgecolor='white', markersize=9, label='双重匮乏'),
...
ax.legend(handles=handles, loc='center left', bbox_to_anchor=(1.02, 0.5), fontsize=16, frameon=True)
fig.savefig(OUT / '图7_补给点Voronoi归属图_美化版.png', bbox_inches='tight')
fig.savefig(OUT / '图7_补给点Voronoi归属图_清晰标记版.png', bbox_inches='tight')
plt.close(fig)
